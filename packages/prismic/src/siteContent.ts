 ...
 *
 * NOTE: In the monorepo each subdomain app is its own Next.js instance,
 * so homePath is "/" for all apps. The `domain` field is used to filter
 * Prismic documents that are tagged for a specific subdomain.
 */
...
async function getBreadcrumbDataForSite(
  siteKey: SiteKey,
  ...
  return { sections, hiddenSegments: finalHiddenSegments };
}
